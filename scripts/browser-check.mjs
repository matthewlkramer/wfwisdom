...
try {
  ...
  await p.click(".job-card ul a >> nth=0"); await p.waitForSelector("h1"); const cards = await p.locator(".wf-card-record").count(); cards > 0 ? ok(`[tl] sub-job page lists ${cards} items`) : bad("[tl] sub-job page empty");

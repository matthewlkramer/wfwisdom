...
try {
  ...
  await p.goto(base); await p.waitForSelector(".stage-picker", { timeout: 20000 }); ok("[tl] home loads with stage picker");
  if (!(await p.locator(".stage-picker button.active:has-text('Planning')").count())) await p.click(".stage-picker button:has-text('Planning')");
  await p.waitForSelector("text=You are in Planning", { timeout: 15000 }); ok("[tl] stage set to Planning; Start here shown"); await p.screenshot({ path: `${shots}/01-home.png`, fullPage: true });
  ...
  await p.goto(`${base}/search?q=how%20do%20we%20set%20tuition%20levels`); await p.waitForSelector(".wf-card-record, .wf-state:not(:has(.wf-spin))", { timeout: 90000 }); const sr = await p.locator(".wf-card-record").count(); sr > 0 ? ok(`[tl] search returned ${sr} results`) : bad("[tl] search returned nothing"); (await p.locator(".item-why").count()) === 0 ? ok("[tl] search results carry no why-this-matches line") : bad("[tl] search still renders why lines"); await p.screenshot({ path: `${shots}/04-search.png`, fullPage: true });
  ...
  await p.fill("input[aria-label='Your question']", "Do I need a liquor license to serve wine at a fundraiser?"); await p.click("button:has-text('Ask')"); await p.waitForFunction(() => document.querySelectorAll(".chat-msg.assistant:not(.muted)").length >= 2, null, { timeout: 90000 }); (await p.locator(".chat-msg.assistant.uncovered").count()) > 0 ? ok("[tl] chat says plainly when Connected does not cover a question") : bad("[tl] chat did not flag an uncovered question"); await p.screenshot({ path: `${shots}/05-ask.png`, fullPage: true });
  await p.goto(`${base}/materials`); await p.waitForSelector("h1:has-text('Draft documents')"); const types = await p.locator("a.wf-card-button").count(); types >= 20 ? ok(`[tl] ${types} material types listed`) : bad(`[tl] only ${types} material types`);
  ...
  await s.goto(`${base}/admin/taxonomy`); await s.waitForSelector("text=Add a job"); ok("[staff] taxonomy structure editor loads");
  await s.click("button:has-text('Move resources')"); await s.waitForSelector(".taxonomy-tree", { timeout: 20000 }); const subs = await s.locator(".taxonomy-tree-sub").count(); subs > 0 ? ok(`[staff] taxonomy tree lists ${subs} sub-jobs to drop onto`) : bad("[staff] taxonomy tree has no sub-jobs");
  await s.click(".taxonomy-tree-sub >> nth=0"); await s.waitForSelector(".taxonomy-item, .wf-state", { timeout: 20000 }); ok("[staff] picking a sub-job lists its resources");

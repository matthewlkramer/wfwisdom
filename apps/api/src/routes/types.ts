...
import { itemSelect, languageWhere, toSummary, type ItemRow } from "../services/items.js";
...
/** The reader's language filter: from the query string, else what they last chose. */
const requestLanguage = (req: { query: Record<string, unknown>; user: { id: string } }): Promise<ResourceLanguage> =>
...
typesRouter.get("/:key/suggested", async (req, res) => {
  ...
  const lang = await requestLanguage({ query: req.query as Record<string, unknown>, user: req.user! });
  const key = `${req.user!.id}:${t.id}:${lang}:${sha(school.text)}`;

...
mapRouter.get("/item/:id", async (req, res) => {
  const staff = req.user!.role === "staff";
  const db = getDb();
  const [r] = await db.select({ ...itemSelect, authorName: sql<string | null>`coalesce(${itemMeta.displayAuthor}, ${items.authorName})`, publishedAt: items.publishedAt, categories: items.categories, audiences: items.audiences, linkedDocs: items.linkedDocs, attachmentsFull: items.attachments, bodyHtml: items.bodyHtml, bodyText: items.bodyText, childPostIds: items.childPostIds, childItemIds: items.childItemIds, sourceKind: items.sourceKind, sourceId: items.sourceId, nativeKind: items.nativeKind, googleKind: items.googleKind, googleFileId: items.googleFileId, authorUserId: items.authorUserId, status: items.status, bodyMarkdown: items.bodyMarkdown, nativeAttachments: items.nativeAttachments, importedFrom: items.importedFrom, importError: items.importError }).from(items).leftJoin(itemMeta, eq(itemMeta.itemId, items.id)).where(and(eq(items.id, String(req.params.id)), visibleWhere(staff)));

...
import { isResourceLanguage, parseRegionFilter, type SessionUser } from "@wfw/shared";
...
export async function resolveUser(req: Request, _res: Response, next: NextFunction): Promise<void> {
  ...
  try {
    const id = unsign(parseCookies(req)[COOKIE]);
    if (id) {
      const rows = await getDb().select({ id: users.id, email: users.email, name: users.name, role: users.role, resourceLanguage: users.resourceLanguage, resourceRegions: users.resourceRegions }).from(sessions).innerJoin(users, eq(sessions.userId, users.id)).where(and(eq(sessions.id, id), gt(sessions.expiresAt, new Date()))).limit(1);
      const u = rows[0];
      if (u) req.user = { id: u.id, email: u.email, name: u.name, role: u.role === "staff" ? "staff" : "teacher_leader", resourceLanguage: isResourceLanguage(u.resourceLanguage) ? u.resourceLanguage : "all", resourceRegions: parseRegionFilter(u.resourceRegions) };
    }

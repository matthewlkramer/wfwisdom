...
export const itemSelect = {
  ...
  updatedAt: items.sourceUpdatedAt, views: items.views, linkOnly: items.linkOnly, attachments: items.attachments, seriesTitles: items.seriesTitles,
...
export type ItemRow = { id: string; title: string; url: string; kind: string; description: string | null; summary: string | null; contentType: string | null; updatedAt: Date | null; views: number; linkOnly: boolean; attachments: { id: number }[]; seriesTitles: string[]; score: number; curation: string | null; hidden: boolean; dated: string | null; reviewStatus: string | null };

export function toSummary(r: ItemRow, why?: string | null): ItemSummary {
  return { id: r.id, title: r.title, url: r.url, kind: r.kind as ItemSummary["kind"], description: r.description, summary: r.summary, contentType: r.contentType, updatedAt: r.updatedAt?.toISOString() ?? null, views: r.views, score: Math.round(r.score), curation: (r.curation as ItemSummary["curation"]) ?? null, dated: r.dated ?? null, linkOnly: r.linkOnly, attachmentCount: r.attachments.length, seriesTitles: r.seriesTitles, why: why ?? null };
}
...
export async function itemsForSubjob(subjobId: string, staff: boolean): Promise<ItemSummary[]> {
  const db = getDb();
  const rows = await db.select({ ...itemSelect, position: placements.position, why: placements.why, isPrimary: placements.isPrimary }).from(placements).innerJoin(items, eq(items.id, placements.itemId)).leftJoin(itemMeta, eq(itemMeta.itemId, items.id)).where(and(eq(placements.subjobId, subjobId), visibleWhere(staff)));
  return rows.sort(rank).map((r) => toSummary(r as ItemRow, r.why));
}
export async function startHere(stage: StageKey, staff: boolean, cap: number): Promise<ItemSummary[]> {
  const db = getDb();
  const pinned = await db.select({ ...itemSelect, position: itemMeta.pinnedPosition }).from(items).innerJoin(itemMeta, eq(itemMeta.itemId, items.id)).where(and(eq(itemMeta.pinnedStage, stage), visibleWhere(staff))).orderBy(itemMeta.pinnedPosition);
  ...
  if (out.length < cap) {
    ...
    const ess = await db.selectDistinctOn([items.id], { ...itemSelect }).from(items).innerJoin(itemMeta, eq(itemMeta.itemId, items.id)).innerJoin(placements, eq(placements.itemId, items.id)).innerJoin(subjobs, eq(subjobs.id, placements.subjobId)).innerJoin(jobs, eq(jobs.id, subjobs.jobId))
      .where(and(sql`${itemMeta.curation} in ('essential','recommended')`, sql`${stage} = any(${subjobs.stages})`, eq(jobs.hidden, false), eq(jobs.staffOnly, false), visibleWhere(staff)));
    for (const e of ess.sort(rank)) if (!ids.has(e.id)) { out.push(e); ids.add(e.id); }
  }
  return out.slice(0, cap).map((r) => toSummary(r));
}
export async function mostUsed(staff: boolean, limit = 8): Promise<ItemSummary[]> {
  const db = getDb();
  const rows = await db.select({ ...itemSelect, clicks: itemMeta.wfClicks30d }).from(items).innerJoin(itemMeta, eq(itemMeta.itemId, items.id)).innerJoin(placements, and(eq(placements.itemId, items.id), eq(placements.isPrimary, true))).innerJoin(subjobs, eq(subjobs.id, placements.subjobId)).innerJoin(jobs, eq(jobs.id, subjobs.jobId))
    .where(and(visibleWhere(staff), eq(jobs.staffOnly, false), sql`${itemMeta.datedLabel} is null`)).orderBy(desc(sql`coalesce(${itemMeta.wfClicks30d},0) * 10 + ${items.views} / greatest(1, extract(epoch from (now() - coalesce(${items.publishedAt}, now()))) / 2592000)`)).limit(limit);
  return rows.map((r) => toSummary(r as ItemRow));
}
export async function itemsByIds(ids: string[], staff: boolean): Promise<Map<string, ItemRow>> {
  if (!ids.length) return new Map();
  const rows = await getDb().select(itemSelect).from(items).leftJoin(itemMeta, eq(itemMeta.itemId, items.id)).where(and(inArray(items.id, ids), visibleWhere(staff)));

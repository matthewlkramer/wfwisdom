...
export function Home() {
  const qc = useQueryClient(); const nav = useNavigate(); const [q, setQ] = useState(""); const { language } = useLanguage(); const { types: docTypes } = useDocTypes();
  const home = useQuery({ queryKey: ["home", language, docTypes], queryFn: () => api.get<HomeData>(`/api/map/home?${langParam(language)}&${typesParam(docTypes)}`) });
  ...
  return (
    <div className="wf-page">
      ...
      {d.stage ? <><div className="wf-section-header"><div><h2>Start here</h2><p>Staff-curated for {stageName}.</p></div><div className="inline-actions filter-group"><TypeFilter /><LanguageFilter /><Link className="secondary-button" to="/map">Browse the whole map</Link></div></div>{d.startHere.length ? <ItemGrid items={d.startHere} context={{ from: "start_here", stage: d.stage }} /> : <State kind="empty" title="Nothing curated for this stage yet">Staff can pin items to a stage from the curation page. Meanwhile, the map and search cover everything.</State>}</> : null}
      <div className="wf-section-header"><div><h2>Most used</h2><p>What the network is opening this month.</p></div>{!d.stage ? <div className="filter-group"><TypeFilter /><LanguageFilter /></div> : null}</div>

...
export function Home() {
  const qc = useQueryClient(); const nav = useNavigate(); const [q, setQ] = useState(""); const { language } = useLanguage();
  const home = useQuery({ queryKey: ["home", language], queryFn: () => api.get<HomeData>(`/api/map/home?${langParam(language)}`) });
    ...
    <div className="wf-page">
      <div className="hero">
        <div className="hero-copy">
          <p className="eyebrow">Connected, organized for the work</p>
          ...
          <SearchInput large value={q} onChange={setQ} placeholder="Search Connected, e.g. how do we set tuition levels" onSubmit={() => q.trim() && nav(`/search?q=${encodeURIComponent(q.trim())}`)} />
          <p className="muted" style={{ marginTop: 8, fontSize: ".85rem" }}>Or <Link to="/ask">ask a question</Link> and get an answer with citations.</p>
          <div style={{ marginTop: 12 }}><LanguageFilter /></div>
        </div>
      ...
      {d.stage ? <><div className="wf-section-header"><div><h2>Start here</h2><p>Staff-curated for {stageName}.</p></div><Link className="secondary-button" to="/map">Browse the whole map</Link></div>{d.startHere.length ? <ItemGrid items={d.startHere} context={{ from: "start_here", stage: d.stage }} /> : <State kind="empty" title="Nothing curated for this stage yet">Staff can pin items to a stage from the curation page. Meanwhile, the map and search cover everything.</State>}</> : null}
      <div className="wf-section-header"><div><h2>Most used</h2><p>What the network is opening this month.</p></div></div>

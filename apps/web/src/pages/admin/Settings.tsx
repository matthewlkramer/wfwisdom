...
export function AdminSettings() {
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); save.mutate(form); }} style={{ display: "grid", gap: 18 }}>
      ...
      <section className="wf-card wf-card-section"><h2>Limits and cost controls</h2><div className="field-row">{num("reviewsPerAccountPerDay", "Reviews per account per day")}{num("reviewsPerDayGlobal", "Reviews per day, whole site")}{num("chatTurnsPerAccountPerDay", "Chat turns per account per day")}{num("chatTurnsPerDayGlobal", "Chat turns per day, whole site")}{num("maxUploadBytes", "Max upload size (bytes)")}{num("maxDraftChars", "Max draft length (characters)")}{num("maxReviewOutputTokens", "Max review output tokens")}</div><p className="muted" style={{ marginTop: 10, fontSize: ".85rem" }}>Worst-case daily review spend at these limits: about ${worst} (every review at the caps on {form.reviewModel}). Typical use is a small fraction of that.</p></section>

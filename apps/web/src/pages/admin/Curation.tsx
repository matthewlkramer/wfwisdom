...
type Row = ItemSummary & { displayTitle: string | null; displayAuthor: string | null; displayDescription: string | null; displayPublishedAt: string | null; hidden: boolean; pinnedStage: string | null; pinnedPosition: number | null; staffNote: string | null; reviewStatus: string | null; hasText: boolean; placements: { key: string; name: string; isPrimary: boolean; position: number | null }[] };
...
function RowView({ r, open, setOpen, subs, stages, filterSubjob, onMeta, onPlace, onRetype, onRegions }: { r: Row; open: boolean; setOpen: () => void; subs: { key: string; name: string; jobName: string }[]; stages: { key: StageKey; name: string }[]; filterSubjob: { key: string; name: string } | null; onMeta: (b: Record<string, unknown>) => void; onPlace: (p: { subjobKey: string; isPrimary: boolean }[]) => void; onRetype: (t: string | null) => void; onRegions: (r: string[]) => void }) {
  const [pl, setPl] = useState(r.placements.map((p) => ({ subjobKey: p.key, isPrimary: p.isPrimary }))); const [addKey, setAddKey] = useState(""); const [note, setNote] = useState(r.staffNote ?? ""); const [dated, setDated] = useState(r.dated ?? ""); const [title, setTitle] = useState(r.displayTitle ?? ""); const [author, setAuthor] = useState(r.displayAuthor ?? ""); const [desc, setDesc] = useState(r.displayDescription ?? ""); const [pub, setPub] = useState((r.displayPublishedAt ?? "").slice(0, 10));
  ...
  return <>
    ...
    {open ? <tr><td colSpan={8} style={{ background: "#f7faf8" }}>
      <div style={{ display: "grid", gap: 10 }}>
        ...
        <div className="field-row"><label className="field"><span>Date shown to readers (leave empty to use the one from Connected)</span><div className="inline-actions"><input type="date" value={pub} onChange={(e) => setPub(e.target.value)} /><button className="small" onClick={() => onMeta({ displayPublishedAt: pub || null })}>Save</button></div></label><label className="field"><span>Description shown to readers (empty shows none; clear the box and Save to go back to Connected’s)</span><div className="inline-actions"><input value={desc} onChange={(e) => setDesc(e.target.value)} placeholder={r.description ?? ""} /><button className="small" onClick={() => onMeta({ displayDescription: desc })}>Save</button><button className="small" onClick={() => { setDesc(""); onMeta({ displayDescription: null }); }}>Use Connected’s</button></div></label></div>

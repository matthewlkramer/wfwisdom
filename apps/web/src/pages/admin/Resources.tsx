...
type Detail = { item: { id: string; title: string; description: string | null; url: string; status: string; nativeKind: Kind; googleKind: string | null; googleFileId: string | null; driveMime: string | null; bodyMarkdown: string | null; childItemIds: string[]; nativeModifiedAt: string | null; language: string }; versions: { id: string; version: number; title: string; createdAt: string; createdBy: string | null; sourceModifiedAt: string | null; chars: number }[]; placements: { key: string; name: string; jobName: string; isPrimary: boolean }[]; stages: string[]; children: { id: string; title: string }[] };
    ...
    <div className="resource-editor">
      <div style={{ display: "grid", gap: 16 }}>
        <div className="wf-card wf-card-section" style={{ display: "grid", gap: 12 }}>
          ...
          {k === "text" ? <label className="field"><span>Text (Markdown)</span><textarea value={md} onChange={(e) => setMd(e.target.value)} /></label> : null}
